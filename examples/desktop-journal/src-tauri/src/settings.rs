...
//!
//! Lives at `$APP_DATA/com.sqlrite.journal/settings.json` — same
//! directory as `journal.sqlrite`, but a separate file so the key
//! never rides along if the user copies or syncs the journal DB.
//!
...
pub struct AskSettings {
    ...
    /// Override `claude-sonnet-4-6` if set.
    #[serde(default)]
    pub model: Option<String>,
    /// Override the engine's default 1024.
    #[serde(default)]
...
mod tests {
    ...
    fn tempdir() -> std::path::PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap_or(0);
        p.push(format!("sqlrite-journal-test-{pid}-{nanos}"));
