...
- **✅ 7g.8 — MCP server `ask` tool.** Lives inside the Phase 7h crate as `sqlrite-mcp/src/tools/ask.rs`. ~190 LOC: builds an `AskConfig` from `SQLRITE_LLM_*` env (with per-call overrides for `model` / `max_tokens` / `cache_ttl`), calls `sqlrite::ask::ask_with_database`, returns `{ sql, explanation, usage, executed }`. Optional `execute: true` arg also runs the generated SQL inline (SELECT → rows; DML/DDL → status, suppressed under `--read-only`). Gated behind the crate's default-on `ask` cargo feature so a `--no-default-features` build produces a six-tool MCP binary with no LLM machinery in the dep tree.
